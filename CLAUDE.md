...
- `src/content/portadas.json` — a `data` collection (Astro's `file()` loader, declared in `content.config.ts` as `portadas`) listing every cover: `{ number?, title, cover }`. `cover` is a path relative to `portadas.json` itself (e.g. `../assets/portadas/foo.webp`) resolved by the schema's `image()` helper, **not** a public URL string — this is what lets `<Image>` optimize it at build time (resized/re-encoded `.webp` variants, `srcset`, intrinsic `width`/`height`). `src/components/CoverGallery.astro` does `getCollection("portadas")` and renders each one with `astro:assets`' `<Image src={cover.data.cover} ... />`; used by `numeros-anteriores.mdx`. It was generated once from the filenames in `public/img/portadas/` (see the naming convention note below) before those files were moved into `src/assets/portadas/` — add/edit/reorder covers by editing this JSON directly.
- `src/assets/portadas/` — the actual cover image files, imported/optimized through the `portadas` collection above (never referenced by a literal `/img/...` URL). Filenames follow the loose convention `<slug>---Actuel-Marx-<N>_1800x.webp` (the trailing `-<N>` is what let the one-time generation script parse the issue number); keep new covers named this way if you ever need to regenerate `portadas.json` from the folder.
- `public/img/` — assets that are served as-is, unoptimized (currently just `logo.jpg`, used directly in `Header.astro`). Anything that benefits from Astro's image pipeline (resizing, format conversion) belongs in `src/assets/` instead, referenced through a content collection's `image()` schema field (like `portadas`) or a direct `import` + `astro:assets`' `<Image>`/`<Picture>`, not in `public/`.
...
- `astro.config.mjs` registers the `mdx`, `sitemap`, and `solid-js` integrations. The site is deployed as a GitHub Pages **project site** at `https://ramosmerino.github.io/landing-actuel-marx/` (`site`/`base` are set accordingly) — not at a bare domain root. Because of that, every internal `href`/`src` that's a literal string (nav links, footer links, the logo in `public/`, favicon, entry list/detail links) is wrapped in `withBase()` from `src/lib/url.ts` instead of being a literal `"/..."` string; a literal absolute path would silently 404 in production since it'd resolve to the domain root instead of `/landing-actuel-marx/...`. Follow the same pattern (`withBase("/foo")`) for any new internal link or `public/`-served asset path. This does **not** apply to images going through `astro:assets` (like the `portadas` collection/`<Image>`) — Astro's own asset pipeline already bakes `base` into the URLs it emits. `.github/workflows/build.yml` builds and deploys to Pages on every push to `main` — GitHub Pages itself still needs to be enabled once in the repo's Settings → Pages → Source: "GitHub Actions" (that toggle can't be set from the codebase).
