...
- `astro.config.mjs` registers the `mdx`, `sitemap`, and `solid-js` integrations. The site is deployed as a GitHub Pages **project site** at `https://ramosmerino.github.io/landing-actuel-marx/` (`site`/`base` are set accordingly) — not at a bare domain root. Because of that, every internal `href`/`src` in the codebase (nav links, footer links, the logo, cover images, favicon, entry list/detail links) is wrapped in `withBase()` from `src/lib/url.ts` instead of being a literal `"/..."` string; a literal absolute path would silently 404 in production since it'd resolve to the domain root instead of `/landing-actuel-marx/...`. Follow the same pattern (`withBase("/foo")`) for any new internal link or asset path. `.github/workflows/build.yml` builds and deploys to Pages on every push to `main` — GitHub Pages itself still needs to be enabled once in the repo's Settings → Pages → Source: "GitHub Actions" (that toggle can't be set from the codebase).
